...
export default function MemberCard(props: MemberCardProps) {
  ...
  return (
    <ReactCardFlip isFlipped={flip} flipDirection="horizontal">
      ...
      {/* back card */}
      <div
        className="m-1.5 cursor-pointer h-full pb-3"
        onClick={() => setFlip(!flip)}>
        <div className="rounded-2xl bg-gray-300 h-full">
          <div className="text-small my-auto py-3 px-4 flex-col items-start">
            <h4 className="font-poppins font-semibold text-xl sm:mb-8 h-16 md:h-0">
              {props.name}
            </h4>
            <p className="sm:mt-2 py-1 text-sm">{props.jikoshoukai}</p>
          </div>
        </div>
      </div>
    </ReactCardFlip>
